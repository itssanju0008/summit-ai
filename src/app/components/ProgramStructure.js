import { Clock, MapPin, Calendar, Users, Presentation, Coffee, Award, UsersRound, Speech, MicVocal } from 'lucide-react';
...
const schedule = [
  {
    ...
    sessions: [
      ...
      },
      {
        ...
        description: "Community-Led Digital Transformation | Tech Entrepreneurship | Policy & Governance",
        icon: Presentation,
        type: "Workshop"
      },
      {
        ...
        description: "Theme Based Sessions Democratic | Responsive | Prioritization",
        icon: Award,
        type: "Thematic"
      }
    ]
  },
  {
    ...
    sessions: [
      ...
      },
      {
        ...
        description: "Presentation of outcomes, vote of thanks, and way forward",
        icon: Award,
        type: "Collaborative"
      },
      {
        ...
        <div className="grid sm:grid-cols-2 gap-4 md:gap-6 mb-8 md:mb-12">
          <div className="bg-slate-50 rounded p-4 md:p-6 border-2 border-amber-600">
            <div className="flex items-center gap-3 md:gap-4">
              ...
              <div className="min-w-0">
                <h3 className="font-serif font-bold text-gray-900 text-base md:text-lg">Duration</h3>
                <p className="text-gray-700 text-xs md:text-sm break-words">1.5 Days | 7th & 8th February 2026</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-slate-50 rounded p-4 md:p-6 border-2 border-amber-600">
            <div className="flex items-center gap-3 md:gap-4">
              ...
              <div className="min-w-0">
                <h3 className="font-serif font-bold text-gray-900 text-base md:text-lg">Venue</h3>
                <p className="text-gray-700 text-xs md:text-sm break-words">Seminar Hall | IIT Delhi</p>
              </div>
            </div>
          </div>
        </div>
            ...
            <div key={dayIndex}>
              ...
              <div className="bg-amber-600 rounded p-4 md:p-6 mb-4 md:mb-6 text-white border-4 border-amber-700">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 md:gap-4">
                  <div>
                    <h3 className="text-2xl md:text-3xl font-serif font-bold mb-1 md:mb-2">{day.day}</h3>
                    <p className="text-white/90 text-sm md:text-base">{day.date}</p>
                  </div>
                  <div className="flex items-center gap-2 bg-white/20 backdrop-blur-sm px-3 py-1.5 md:px-4 md:py-2 rounded border border-white/30 text-sm md:text-base">
                    ...
                    <span className="font-semibold whitespace-nowrap">
                      {day.sessions[0].time.split(' - ')[0]} - {day.sessions[day.sessions.length - 1].time.split(' - ')[1]}
                    </span>
                  </div>
                </div>
              </div>
                    ...
                    >
                      <div className="flex flex-col sm:flex-row gap-3 md:gap-4 sm:gap-6">
                        ...
                        <div className="flex sm:flex-col items-center sm:items-start gap-3 sm:gap-0 flex-shrink-0">
                          <div className={`w-12 h-12 md:w-16 md:h-16 ${bgColor} rounded flex items-center justify-center sm:mb-2 md:mb-3`}>
                            <Icon className="w-6 h-6 md:w-8 md:h-8 text-white" />
                          </div>
                          <div className="sm:text-center">
                            <span className={`inline-block ${bgColor} text-white px-2 py-0.5 rounded text-[10px] sm:text-xs font-semibold uppercase tracking-wide`}>
                              {session.type}
                            </span>
                          </div>
                        </div>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 sm:gap-4 mb-2 md:mb-3">
                            <h4 className="text-base md:text-lg font-serif font-bold text-gray-900 break-words">{session.title}</h4>
                            <span className="text-amber-700 font-semibold text-xs md:text-sm whitespace-nowrap flex items-center gap-1 md:gap-2">
                              ...
                              <span className="break-all sm:break-normal">{session.time}</span>
                            </span>
                          </div>
                          <p className="text-sm md:text-base text-gray-700 leading-relaxed break-words">{session.description}</p>
                        </div>
                      </div>
                    </div>
                  );
